...
contract WalletV1 is Ownable, ERC20, IERC20 {
    ...
    function sendETH(address payable to, uint weiAmount) public onlyOwner {
        require(
            address(this).balance >= weiAmount + ethFee,
    ...
    function sendTokens(address token, address to, uint256 amount) public {
        ...
        if (msg.sender != owner()) {
            require(
                allowances[token][msg.sender] >= totalAmount,
                "Insufficient allowance"
            );
            allowances[token][msg.sender] = allowances[token][msg.sender].sub(
                totalAmount
            );
        }
